/*
    ...
    데이터 공급자(Provider)와 소비자(Consumer)를 정의하여 데이터가 필요한 컴포넌트만 사용할수 있게 구현할 수 있다.

    컨텍스트를 사용하면 하위컴포넌트가 여러개인 구조에서 유용하게 사용할 수 있음.
    몇번째 하위 컴포넌트인지 상관없이 필요한 하위컴포넌트에서 소비자를 임포트해서 필요한 데이터를 사용할 수 있음

    props vs Context
    props는 부모컴포넌트에서 자식컴포넌트에게만 전달되는 데이터라면
    Context는 모든 컴포넌트에서 전반적으로 영향을 끼칠 수 있다.
*/
...
class ContextApi extends React.Component{
    ...
    render() {
        ...
        return(
            //자식태그(Children)를 Provider로 감싸고 전달할 데이터를 value값을 할당
