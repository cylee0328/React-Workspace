...
/*
    콜백함수?
    자바스크립트의 함수중 몇몇 코드는 중 비동기적으로 작동하는 코드는 
    실행된 작업이 끝나지 않았더라도 다음 작업이 시작될 수 있는데, 이때 특정코드를 내가 원하는 시점에
    실행될 수 있도록 컨트롤해줘야할때 콜백함수를 사용한다.
    (일반함수 -> stack, ㄴ비동기함수 -> queqe)

    Promise?
    콜백함수와 마찬가지로 비동기적으로 동작하는 코드를 동기적으로 구현할때 사용한다. 차이점은 콜백함수에 비해서
    가독성을 높일 수 있고, 예외처리를 쉽게 처리할 수 있다.
*/
...
class Callback extends Component{
    ...
    componentDidMount(){
        ...
        this.logPrint(1, function(return1){
            ...
            /*
                ...
                하지만 콜백함수를 여러번 사용할 수록 코드가 더 지저분해진다는 단점도 존재함. 이때 대체제로 나온
                기능이 Promise
            */
        }.bind(this)
        /*
            화살표함수가 아닌 익명함수는 this가 window객체로 바인드되기때문에 this가 현재 객체를 따라갈 수 있도록
            바인드 메서드를 사용함
            익명함수는 this가 동적으로 바인딩되고(호출한 객체에 바인딩)
            arrow함수는 this가 정적으로 바인딩된다.
        */ 
        );
    }
    ...
    logPrint(param, callback){
        ...
        callback(param); // 매개변수로 전달받은 함수의 파라미터로 다시 넣고 함수를 실행
    }
    ...
    componentDidMount(){
       ...
       new Promise(
            resolve => {
                setTimeout( function(){
                    resolve('promise');
                    // 1.5초 후에 resolve함수가 실행되며 이때 이행상태로 변경이되면서 
...
class PromiseCatch extends Component{
    ...
    componentDidMount(){
        ...
        /*
            reject? 대기, 이행, 거부 중 거부상태가 됐을때 catch라는 함수 실행한다.
